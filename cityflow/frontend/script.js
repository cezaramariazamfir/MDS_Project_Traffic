...
const nodeCanvas = document.getElementById("simulator-canvas")
const replayControlDom = document.getElementById("replay-control")
const replaySpeedDom = document.getElementById("replay-speed")
...
async function fetchFile(filePath) {
  try {
    ...
    infoAppend(`Error fetching ${filePath}: ${error.message}`)
    throw error
  }
}

// Auto-start function that loads files from parent directory
async function autoStart() {
  ...
  try {
    infoAppend("Loading roadnet_log.json")
    const roadnetContent = await fetchFile("roadnet_log.json")
    roadnetData[0] = roadnetContent
    infoAppend("roadnet_log.json loaded successfully")

    infoAppend("Loading replay_log.txt")
    const replayContent = await fetchFile("replay_log.txt")
    replayData[0] = replayContent
    infoAppend("replay_log.txt loaded successfully")

    // Continue with the same processing as in the start() function
    ...
    infoAppend("Auto-loading files failed: " + error.message)
    console.error(error)
    loading = false
    // Show the file upload interface as fallback
    document.getElementById("guide").classList.remove("d-none")
  }
}
...
let chartLog
let showChart = false
const chartConainterDOM = document.getElementById("chart-container")
function start() {
  if (loading) return
  loading = true
  infoReset()
  uploadFile(roadnetData, RoadnetFileDom.files[0], () => {
    uploadFile(replayData, ReplayFileDom.files[0], () => {
      const after_update = () => {
        infoAppend("drawing roadnet")
        ready = false
        document.getElementById("guide").classList.add("d-none")
        ...
        if (showChart) {
          ...
          const chart_lines = chartData[0].split("\n")
          ...
          for (let i = 0; i < totalStep; ++i) {
            const step_data = chart_lines[i + 1].split(/[ \t]+/)
            chartLog.push([])
            for (let j = 0; j < step_data.length; ++j) {
              chartLog[i].push(Number.parseFloat(step_data[j]))
            }
          }
...
const RoadnetFileDom = document.getElementById("roadnet-file")
const ReplayFileDom = document.getElementById("replay-file")
const ChartFileDom = document.getElementById("chart-file")
...
replayControlDom.addEventListener("change", (e) => {
...
function drawRoadnet() {
  ...
  // 2) inițializează viewport cu dimensiunile corecte
  ...
  const carTexture = renderer.generateTexture(carG)

  const signalG = new Graphics()
  signalG
    .beginFill(TURN_SIGNAL_COLOR, 0.7)
    .drawRect(0, 0, TURN_SIGNAL_LENGTH, TURN_SIGNAL_WIDTH)
    .drawRect(0, 3 * CAR_WIDTH - TURN_SIGNAL_WIDTH, TURN_SIGNAL_LENGTH, TURN_SIGNAL_WIDTH)
    .endFill()
  const turnSignalTexture = renderer.generateTexture(signalG)

  const signalLeft = new Texture(turnSignalTexture, new Rectangle(0, 0, TURN_SIGNAL_LENGTH, CAR_WIDTH))
  const signalStraight = new Texture(turnSignalTexture, new Rectangle(0, CAR_WIDTH, TURN_SIGNAL_LENGTH, CAR_WIDTH))
  const signalRight = new Texture(turnSignalTexture, new Rectangle(0, CAR_WIDTH * 2, TURN_SIGNAL_LENGTH, CAR_WIDTH))
  turnSignalTextures = [signalLeft, signalStraight, signalRight]

  carPool = []
  if (debugMode) carContainer = new Container()
  else carContainer = new ParticleContainer(NUM_CAR_POOL, { rotation: true, tint: true })
  ...
  for (let i = 0, len = NUM_CAR_POOL; i < len; ++i) {
    ...
    const signal = new Sprite(turnSignalTextures[1])
...
function drawNode(node, graphics) {
  ...
  for (let i = 0; i < outline.length; i += 2) {
    ...
    if (i == 0) graphics.moveTo(outline[i], outline[i + 1])
    else graphics.lineTo(outline[i], outline[i + 1])
  }
  graphics.endFill()
...
function drawEdge(edge, graphics) {
  ...
  for (let i = 1; i < points.length; ++i) {
    ...
    for (let lane = 0, len = edge.nLane - 1; lane < len; ++lane) {
      ...
      graphics.drawDashLine(
        pointA.moveAlong(pointAOffset, offset),
        pointB.moveAlong(pointBOffset, offset),
        LANE_DASH,
        LANE_GAP,
      )
    }
...
function drawStep(step) {
  ...
  for (let i = 0, len = carLogs.length - 1; i < len; ++i) {
    ...
    position = transCoord([Number.parseFloat(carLog[0]), Number.parseFloat(carLog[1])])
    length = Number.parseFloat(carLog[5])
    width = Number.parseFloat(carLog[6])
    carPool[i][0].position.set(position[0], position[1])
    carPool[i][0].rotation = 2 * Math.PI - Number.parseFloat(carLog[2])
    carPool[i][0].name = carLog[3]
    const carColorId = stringHash(carLog[3]) % CAR_COLORS_NUM
    carPool[i][0].tint = CAR_COLORS[carColorId]
    carPool[i][0].width = length
    carPool[i][0].height = width
    carContainer.addChild(carPool[i][0])

    const laneChange = Number.parseInt(carLog[4]) + 1
...
const chart = {
  ...
  init: function (title, series_cnt, max_step) {
    ...
    this.data.series = []
    for (let i = 0; i < series_cnt; ++i) this.data.series.push([])
  ...
  clear: function () {
    for (let i = 0; i < this.data.series.length; ++i) this.data.series[i] = []
  },
  ptr: 0,
}
